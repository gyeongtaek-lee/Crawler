...
// customizing 151535
Client.prototype.moveQueue = function(body, _cb) {

	if (!body || !body.vhost) {
		_cb ({
			message : "Node vhost not provided"
		});
		return;
	}
	
	if (!body || !body.srcQueue) {
		_cb ({
			message : "Node srcQueue not provided"
		});
		return;
	}
	
	if (!body || !body.destQueue) {
		_cb ({
			message : "Node destQueue not provided"
		});
		return;
	}
	
	var path = '/api/parameters/shovel/' + encodeURIComponent(body.vhost) + '/' + encodeURIComponent('Move from ') + encodeURIComponent(body.srcQueue);
	var putBody = {
		component	: "shovel",
		vhost			: "/",
		name			: "Move from " + body.srcQueue,
		value			: {
			"src-uri":"amqp:///" + encodeURIComponent(body.vhost),
			"src-queue":body.srcQueue,
			"dest-uri":"amqp:///" + encodeURIComponent(body.vhost),
			"dest-queue":body.destQueue,
			"prefetch-count":1000,
	...
	if ( env !== 'local' ){
		...
		putBody.value["src-protocol"] = "amqp091";
		putBody.value["dest-protocol"] = "amqp091";
		putBody.value["src-delete-after"] = "queue-length";
	}else{
		// local (http://10.203.7.214:15672/) : rabbitmq 3.6.0
		putBody.value["delete-after"] = "queue-length";
	}
	
//	console.log( path );
//	console.log( putBody );
	
	this.putClient.makeRequest(path, putBody,  _cb);
}


//customizing 151535
Client.prototype.deleteShovel = function(body, _cb) {
